...
module axle_mounting_bracket_2d() {
  difference() {
    polygon(points = [[5,5], [-5,5], [-5,-5], [-15,-5],[-15,2],[-25,2], [-25,-15], [5,-15]]);
    circle(d=3);
  }
}
...
module diverter_assembly()
{
  ...
  translate([0,0,55])
    ...
    translate([0,1.5,-55]) 	rotate([90,0,0]) linear_extrude(height=3) diversion_plate_2d();
...
// fixed conduit
for(i=[0:columns-1]) translate([i*pitch, -14,60]) cube([16,10,50]);
