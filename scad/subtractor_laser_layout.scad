...
offset(delta = kerf, chamfer = true) {
  ...
  translate([10,200]) {
    ...
    for(i=[0:7]) translate([170+20*i,30]) input_guard_a_2d();
    for(i=[0:7]) translate([20+45*i,90]) input_guard_b_2d();
    for(i=[0:3]) {
      translate([320+15*i,30]) output_toggle_2d();
      translate([320+7.5+15*i,60]) rotate(180) output_toggle_2d();
    }
    for(i=[0:7]) translate([160+20*i,25]) rotate(-10)reset_toggle_2d();
  }
